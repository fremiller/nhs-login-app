    ...
    render() {
        ...
        return (
            <View style={styles.mainView}>
                {this.state.loading ? <ActivityIndicator color="#0000FF"></ActivityIndicator> : <></>}
                {this.state.chatError != "" ? <NhsCard title="Error Fetching Chats" body={this.state.chatError}></NhsCard> : <></>}
                {!NhsLogin.instance.chatEnabled ? <NhsCard title="Chat Disabled" body={NhsLogin.instance.chatDisabledMessage != "" ? NhsLogin.instance.chatDisabledMessage : "Chat functionality has been disabled on the app server."}></NhsCard> : <></>}
                {this.state.chats.length > 0 ? this.state.chats.map((chat) => {
                    return (
                        <ChatButton key={chat.gp.id} imageSrc={chat.gp.image} name={chat.gp.name} location={chat.gp.location} id={chat.gp.id} onPress={(chat) => {
